...
이 저장소에는 **두 세대의 UI가 함께 있습니다.**

| 위치 | 상태 |
|---|---|
| **`MAP/`** | **현재 제품 UI와 지도 엔진.** 오지 생존 도메인. 여기를 보시면 됩니다 |
| 루트 `index.html` · `js/` · `UI_적용법.md` | **구 의료 도메인 TEST UI.** 교체 대기 중 |

프로젝트는 의료 응급처치에서 오지 생존으로 도메인을 전환했습니다.
루트의 TEST UI에는 전환 이전 개념(의약품 수납칸 A~D, 구조 문자, 사진 위험 판정)이 남아 있으며,
이들은 전부 **동결된 삭제 항목**입니다. 현재 작품의 기능이 아닙니다.

교체 전까지 오해를 막기 위해 이 사실을 숨기지 않고 적어 둡니다.

## 구성

```text
server.py          키오스크 프록시 (:8780) — backend(:8765)를 중계
index.html         ⚠️ 구 의료 도메인 TEST UI (교체 대기)
styles.css
js/                ⚠️ 구 의료 도메인 TEST UI (교체 대기)
UI_적용법.md       ⚠️ 위 TEST UI 설명서. 삭제된 하드웨어를 다룹니다
